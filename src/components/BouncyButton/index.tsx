...
export function BouncyButton(props: BouncyButtonProps) {
    ...
    const handleClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        circleControls.start(i => ({
            scaleY: 1.14,
            scaleX: 1,
            transition: { type: 'spring', delay: i * 0.04, duration: 0.05, stiffness: 1000, damping: 20, restDelta: 0.001 },
        }))
        setTimeout(()=>{
            svgRef.current.style.transition = 'transform 0.13s';
            ...
            circleControls.start(i => ({
                scaleY: 1,
                scaleX: 1,
                transition: { type: 'spring', delay: i * 0.02, duration: 0.05, stiffness: 500, damping: 5, restDelta: 0.001 },
            }))
        }, 140);
        onClick(e);
    };
    ...
    const handleHoverStart = () => {
        circleControls.start(i => ({
            scaleY: 1.07,
            scaleX: 0.99,
            transition: { type: 'spring', delay: i * 0.02, duration: 0.05, stiffness: 1000, damping: 20, restDelta: 0.001 },
        }))
    };
