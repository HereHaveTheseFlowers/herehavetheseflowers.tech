...
BlockGrid.Block = function Block(props: BlockPreviewProps) {
  ...
  if (date) {
    ...
    displayedDate = blockDate
      .toLocaleString(localeLang, dateOptions)
      .replace('.', '')
      .toLowerCase();
  ...
  const navigateToBlockPage = () => {
    navigate(
      `${location.pathname.includes('/ru') ? '/ru' : ''}/${translateCategory(props.category, 'en').replaceAll(' ', '-')}/block/${props.id}`
    );
  };
      ...
      <div className='block'>
        <div className='block__status'>
          <div className='block__category'>
            <span className='block__category-span skeleton-box' style={{ width: '30px' }}>
              {'\xa0'}
            </span>
          </div>
        ...
        <div className='block__status'>
          ...
          {pinned && (
            <svg className='block__pinned' viewBox='0 0 800 799.83'>
              <path
                d='M793.18 192.28c19.95-20-5.35-77.75-56.6-129-50.65-50.64-107.48-75.82-128.14-57.1l-.4-.39-283.91 
          ...
          {isHot && (
            <>
              <svg className='block__notification' viewBox='0 0 70.7 70.7'>
                <path
                  d='M28.3,63.2h14c0,0.9-0.2,1.9-0.5,2.7c-0.9,2.1-2.8,3.7-5,4.1c-0.5,0.1-0.9,0.1-1.4,0.1
              ...
              <span className='block__notification-text'>NEW!</span>
            </>
          )}
      ...
      <div className='block block_state_loaded'>
        ...
        <div className='block__status'>
          ...
          {isHot && (
            <>
              <svg
                className='block__notification'
                viewBox='0 0 70.7 70.7'
                onClick={navigateToBlockPage}
              >
                <path
                  d='M28.3,63.2h14c0,0.9-0.2,1.9-0.5,2.7c-0.9,2.1-2.8,3.7-5,4.1c-0.5,0.1-0.9,0.1-1.4,0.1
                                        C31.5,70.2,28.3,67.1,28.3,63.2z M9.8,60.2c-1.4,0-2.5-1.1-2.5-2.5c0-0.7,0.3-1.3,0.7-1.8l4.3-4.3c1.3-1.3,2-3,2-4.9V32.2
              ...
              <span className='block__notification-text' onClick={navigateToBlockPage}>
                NEW!
              </span>
            </>
          )}
          <span className='block__date' onClick={navigateToBlockPage}>
            {displayedDate}
          </span>
        </div>
        <div className='block__thumbnail' onClick={navigateToBlockPage}>
          <img
            className='block__thumbnail-image image_state_loaded'
            src={thumbnailURL}
            alt={`${name} thumbnail`}
          />
        </div>
