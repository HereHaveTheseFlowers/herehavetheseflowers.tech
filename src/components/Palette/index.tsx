...
export const Palette = React.forwardRef((props, ref: any) => {
  ...
  let colorHexInitial = '';
  let colorHslInitial =
    store.getState().colorHSL == !null
      ? store.getState().colorHSL
      : { h: 0, s: defaultStatsMain.saturation, l: defaultStatsMain.lightness };
  ...
  useEffect(() => {
    ...
    if (window.localStorage.getItem('mainColor')) {
      ...
      htmlElem.style.setProperty('--color-main', colorHexInitial);
    }
    /* else if(htmlElem) {
    ...
    if (store.getState().theme === 'dark') {
      hexBG = hslToHex(
        colorHslInitial.h,
        defaultStatsBGDark.saturation,
        defaultStatsBGDark.lightness
      );
    } else {
  ...
  store.on('theme', () => {
    ...
    if (store.getState().theme === 'dark') {
      ...
      hexBGSwitched = hslToHex(
        bgHueSwitched,
        defaultStatsBGLight.saturation,
        defaultStatsBGLight.lightness
      );
    }
    const pattern = /^#[a-zA-Z\d]{6}$/gm;
  ...
  const handleChangeHSL = (e: any) => {
    ...
    if (pattern.test(hex)) {
      ...
      htmlElem.style.setProperty('--color-main', hex);
    }
  ...
  return (
    <div className='palette palette_state_hidden' ref={ref}>
      ...
      <div className='palette__field'>
        <SaturationColorPicker hsl={colorHSL} />
      </div>
    </div>
  );
});
