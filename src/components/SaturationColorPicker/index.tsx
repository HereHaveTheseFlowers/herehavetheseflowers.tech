...
export function SaturationColorPicker(props: SaturationColorPickerProps) {
    ...
    if(pattern.test(hex)) {
        if(gradientElement.current) {
            gradientElement.current.style
                .background = `linear-gradient(to right, #fff 0%, ${hex} 100%)`;
        }
    }
    ...
    useEffect(() => {
        ...
        if(pickerElement?.current && cursorElement?.current) {
            ...
            pickerElement.current.addEventListener("mousemove", (e: MouseEvent) => { handleChangeColor(e) })
            pickerElement.current.addEventListener("mousedown", (e: PointerEvent) => { handleChangeColor(e, true) })
        }
    }, []);
    return <>
        <div style={{position: "absolute", inset: "0px"}} ref={pickerElement}>
            <div className="saturationpicker" style={{padding: "0px 2px", position: "relative", height: "100%"}} ref={gradientElement}>
                <div style={{position: "absolute", left: `${hsl.s}%`}} ref={cursorElement}>
                    <div style={{ marginTop: "1px", width: "4px", borderRadius: "1px", height: "8px", boxShadow: "rgba(0, 0, 0, 0.6) 0px 0px 2px", background: "rgb(255, 255, 255)", transform: "translateX(-2px)" }}></div>
                </div>
            </div>
        </div>
    </>
