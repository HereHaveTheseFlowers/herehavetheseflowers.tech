...
export default function Block() {
    ...
    const [currentBlock, setCurrentBlock] = useState(store.getState().blocks.find((block: BlockProps) => block.id === blockId))
    ...
    if(currentBlock) {
        ...
        if(date) {
            ...
            displayedDate = blockDate.toLocaleString(localeLang, dateOptionsBlock).replace(".", "").toLowerCase();
        ...
        const navigateToBlockCategory= () => {
            if(!category) return
            navigate(`${location.pathname.includes("/ru") ? "/ru" : ""}/${translateCategory(category, "en").replaceAll(" ", "-")}`)
        }
        if(!imageLoading) {
            return (
                <>
                    ...
                    <article className="block-article">
                        <section className="block-article__thumbnail-wrapper">
                            <img className="block-article__thumbnail-image image_state_loaded" src={thumbnailURL} alt={`${name} thumbnail`} draggable="false" />
                        </section>
                        <section className="block-article__info">
                            <div className="block-article__name">
                                {name} <br />
                                { website && <a className="block-article__link" href={website} target="_blank">{website.replace("https://", "")}</a> }
                            </div>
                            <div className="block-article__content">
                                {contentArray.map((paragraph: string, index: number)=> {
                                    return <span key={index}>{paragraph}<br /><br /></span>
                                })}
                            ...
                            <div className="block-article__footer">
                                <div className="block-article__category">
                                    <span className="block-article__category-span underlined" onClick={navigateToBlockCategory}>
                                        {`${location.pathname.includes("/ru") === true ? "категория" : "category" }`}: {category} |
                                    </span>
                                </div>
                                <div className="block-article__date">
                                    <br />
                                    {`${location.pathname.includes("/ru") === true ? "когда" : "when" }`}: {displayedDate.toLowerCase()}
                                </div>
                            </div>
                        </section>
                    </article>
            ...
            return (
                <>
                    ...
                    <article className="block-article">
                        <section className="block-article__thumbnail-wrapper">
                                <img className="block-article__thumbnail-image image_state_unloaded" src={thumbnailURL} alt={`${name} thumbnail`} onLoad={() => {
                                        handleImageLoaded();
                                    }} />
            ...
            <>
                ...
                <article className="block-article">
                    ...
                    <section className="block-article__info">
                        ...
                        <div className="block-article__footer  skeleton-box">
                            {'\xa0'}
                        </div>
                    </section>
                </article>
